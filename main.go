...
const RootServers = "198.41.0.4,199.9.14.201,192.33.4.12,199.7.91.13,192.203.230.10,192.5.5.241,192.112.36.4,198.97.190.53"
...
func main() {
	...
	for {
		...
		go func() {
			err := handlePacket(pc, addr, buffer[:n])
			if err != nil {
				fmt.Printf("Error handling packet from %s: %s\n", addr.String(), err)
			}
		}()
	}
}
